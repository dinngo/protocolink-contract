...
import {IAgent2, AgentImplementation2} from './AgentImplementation2.sol';
import {Agent2} from './Agent2.sol';
import {FeeGenerator2} from './fees/FeeGenerator2.sol';
import {IParam2} from './interfaces/IParam2.sol';
import {IRouter2} from './interfaces/IRouter2.sol';
import {LogicHash2} from './libraries/LogicHash2.sol';
import 'forge-std/console.sol';

/// @title Entry point for Composable Router
contract Router2 is IRouter2, EIP712, FeeGenerator2 {
    using SafeERC20 for IERC20;
    using LogicHash2 for IParam2.LogicBatch;
    ...
    mapping(address user => IAgent2 agent) public agents;
    ...
    constructor(
        ...
        currentUser = _INIT_USER;
        agentImplementation = address(new AgentImplementation2(wrappedNative));
    ...
    function owner() public view override(IRouter2, Ownable) returns (address) {
        return super.owner();
    }
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            keccak256(abi.encodePacked(type(Agent2).creationCode, abi.encode(agentImplementation)))
                        )
                    )
                )
            )
    ...
    ///         Charge fees based on the scenarios defined in the FeeGenerator2 contract, which calculates fees on-chain.
    ...
    function execute(
        ...
        address user = currentUser;
        IAgent2 agent = agents[user];

        if (address(agent) == address(0)) {
            agent = IAgent2(newAgent(user));
        }

        IParam2.Fee[] memory fees = getFeesByLogics(logics, msg.value);
    ...
    function newAgent(address user) public returns (address payable) {
        if (address(agents[user]) != address(0)) {
            ...
            IAgent2 agent = IAgent2(address(new Agent2{salt: bytes32(bytes20((uint160(user))))}(agentImplementation)));
            agents[user] = agent;
