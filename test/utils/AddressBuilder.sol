...
library AddressBuilder {
    ...
    function toPayable(address[] calldata a) external pure returns (address payable[] memory addresses) {
        addresses = new address payable[](a.length);
        for (uint256 i = 0; i < a.length; ++i) {
            addresses[i] = payable(a[i]);
        }
    }
}
