...
contract DelegationTypehash is Test, DelegationSignature {
    ...
    function testDelegationTypehash() external {
        ...
        assertEq(getDelegationSignature(details, _buildDomainSeparator(), PRIVATE_KEY), sig);

        // Verify the signer can be recovered using the external sig
        bytes32 hashedTypedData = getDelegationHashedTypedData(details, _buildDomainSeparator());
